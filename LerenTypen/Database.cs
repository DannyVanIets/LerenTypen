...
namespace LerenTypen
{
    static class Database
    {
        ...
        public static bool DeleteAcc(string userName)
        {
            ...
            {
                connection.Open();
                string query = "DELETE FROM accounts WHERE accountID = @id AND accountUsername = @username";
        ...
        public static string GetAccountUsername(int accountID)
        {
            ...
            {
                connection.Open();
                string query = "SELECT accountUsername FROM accounts WHERE accountID = @id";
        ...
        /// <returns></returns>
        public static int InsertResults(int testID, int accountID, int wordsEachMinute, int pauses, List<string> rightAnswers, Dictionary<int, string> wrongAnswers, List<string> lines, int score)
        {
            int testResultID = 0;
            SqlConnection connection = new SqlConnection(connectionString);
            ...
            try
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        {
                            //add all the found data to a list
                            tests.Add(new TestTable(counter, reader.GetString(2), reader.GetInt32(4), reader.GetInt32(5), GetAmountOfWordsFromTest(reader.GetInt32(0)), reader.GetInt32(3), reader.GetString(6)));
                            counter++;
                        }
                    }
                }
